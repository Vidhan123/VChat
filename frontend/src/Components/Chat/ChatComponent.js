...
function ChatComponent(props) {
  ...
  useEffect(() => {
    ...
    const getDetails = async () => {
      if(chatId) {
        const rooms = await axios.get('http://localhost:9000/getRooms');
        ...
        const rooms = await axios.get('http://localhost:9000/getGroups');
        for(let i=0;i<rooms.data.length;i++) {
          if(rooms.data[i]._id === grpId) {
            ...
            setP2({ name: rooms.data[i].name, dp: `http://localhost:9000/${rooms.data[i].pic}`, email: str1 });
            break;
          }
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    if(file == null) {
      ...
      const url = 'http://localhost:9000/mediaMsg';
        ...
        <Paper elevation={2} square>
          <div className={classes.chatTop}>
            ...
            <div className={classes.chatTopRight}>
              ...
              <MenuItem onClick={async () => {
                handleClose1();
                if(chatId) {
                  await axios.post('http://localhost:9000/deleteConversation',{
                    roomId: chatId
                  });
                }
                else if(grpId) {
          ...
          <div className={msg.senderEmail === email ? 'chatMsgSend' : 'chatMsgGet'} key={msgs.indexOf(msg)+1}>
            <span style={{fontWeight: 800,fontSize: 'small',color: '#3F3D56',display: msg.senderEmail === email || chatId ? 'none' : msg.chatMessage.substring(0,6) !== 'public' ? 'initial' : 'block'}}>{msg.senderName}</span>
            {
              ...
              <video style={{width:'200px',maxWidth: '100%',marginTop: '2px'}} src={`http://localhost:9000/${msg.chatMessage}`} alt='mediaVideo' type='video/mp4' controls />
              :
              ...
              <img style={{width:'200px',maxWidth: '100%',marginTop: '2px'}} src={`http://localhost:9000/${msg.chatMessage}`} alt='mediaImg' /> 
              :
              <a href={`http://localhost:9000/${msg.chatMessage}`} download={msg.chatMessage.substring(7,msg.chatMessage.length)}>{msg.chatMessage.substring(7,msg.chatMessage.length)}</a>
              // <div style={{height: '100%'}}>
              //   <iframe src={`http://localhost:9000/${msg.chatMessage}`} style={{width:'200px',maxWidth: '100%',marginTop: '2px'}} ></iframe>
              // </div>
            }
            <br />
