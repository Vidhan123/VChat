...
function Sidebar(props) {
  ...
  useEffect(() => {
    const getDetails = async () => {
      const resDes = await axios.get('http://localhost:9000/userinfo');
      const rooms = await axios.get('http://localhost:9000/getRooms');
      const grps = await axios.get('http://localhost:9000/getGroups');
      setData(resDes.data);
  ...
  const suggestionSelected = async (value) => {
    // setText(value);
    setSuggestions([]);
    await axios.post('http://localhost:9000/newRoom',{
      p1: email,
      p2: value
    });
    setText('');
    ...
    <div style={modalStyle} className={classes.modal}>
      ...
      <div id="simple-modal-description">
        ...
        <Button variant="contained" color="primary" onClick={async (e) => {
          ...
          const url = 'http://localhost:9000/newGroup';
        ...
        <div className={classes.sidebarTop}>
          ...
          <div className={classes.sidebarTopRight}>
            ...
            <IconButton onClick={async () => {
              await axios.post('http://localhost:9000/deleteAccount',{
                email
              });
              await unauthorise();
              history.push('/');
        ...
        <div className='sidebarChats'>
          {
            groups.map((grp,index) => 
              <a href={`http://localhost:3000/chat/group/${grp._id}`} style={{textDecoration: 'none',color: '#000'}} key={index} >
          ...
          }
          {
            rooms.map((room,index) => 
              <a href={`http://localhost:3000/chat/${room._id}`} style={{textDecoration: 'none',color: '#000'}} key={index} >
